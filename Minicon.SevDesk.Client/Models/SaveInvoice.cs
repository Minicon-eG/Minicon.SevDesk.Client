...
public class SaveInvoice : IEquatable<SaveInvoice>, IValidatableObject
{
		...
		SaveInvoiceDiscountSave discountSave = default, SaveInvoiceDiscountDelete discountDelete = default)
	{
		...
		if (invoice == null)
		{
			throw new InvalidDataException("invoice is a required property for SaveInvoice and cannot be null");
		}

		Invoice = invoice;
		InvoicePosSave = invoicePosSave;
		InvoicePosDelete = invoicePosDelete;
		Filename = filename;
	...
	public bool Equals(SaveInvoice input)
	{
		...
		return
			...
			(
				InvoicePosSave == input.InvoicePosSave ||
				(InvoicePosSave != null &&
				 InvoicePosSave.Equals(input.InvoicePosSave))
			) &&
			(
				InvoicePosDelete == input.InvoicePosDelete ||
				(InvoicePosDelete != null &&
				 InvoicePosDelete.Equals(input.InvoicePosDelete))
			) &&
			(
	...
	public override int GetHashCode()
	{
		unchecked // Overflow is fine, just wrap
		{
			...
			if (InvoicePosSave != null)
			{
				hashCode = hashCode * 59 + InvoicePosSave.GetHashCode();
			}

			if (InvoicePosDelete != null)
			{
				hashCode = hashCode * 59 + InvoicePosDelete.GetHashCode();
			}
